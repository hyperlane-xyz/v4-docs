...
在某些情况下，您可能需要在进行调用之前计算远程链上的 ICA 地址。例如，如果您的 ICA 在执行交易之前需要资金，您可以检索其地址并提前向其转移资产。有关更多信息，请参见[转移和调用模式](/zh/docs/guides/warp-routes/evm/transfer-and-call-pattern)部分。
