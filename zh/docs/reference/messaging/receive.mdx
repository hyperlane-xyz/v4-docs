...
要传递跨链消息，[中继器](/zh/docs/operate/relayer/run-relayer)调用 `Mailbox.process()`。
...
<Info>
发送者地址左填充为 `bytes32`，以兼容地址不同的虚拟机。为方便起见，在 [`TypeCasts` 库](/zh/docs/reference/developer-tools/libraries/typecasts)中提供了以下实用工具。
...
为方便起见，在 [`MailboxClient` 库](/zh/docs/reference/developer-tools/libraries/mailbox-client)中提供了以下实用工具。
...
当接收到消息时，邮箱将在调用消息接收者的 `handle` 之前使用[跨链安全模块](/zh/docs/protocol/ISM/modular-security)验证安全性。
