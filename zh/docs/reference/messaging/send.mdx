...
<Info>
接收者地址左填充为 `bytes32`，以兼容地址不同的虚拟机。为方便起见，在 [`TypeCasts` 库](/zh/docs/reference/developer-tools/libraries/typecasts)中提供了以下实用工具。
...
要在 `dispatch` 调用中使用自定义钩子覆盖默认钩子，请参见[钩子参考](/zh/docs/reference/hooks/interchain-gas)。