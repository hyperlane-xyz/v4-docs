...
- 如果接收者未实现 `ISpecifiesInterchainSecurityModule` 或 `recipient.interchainSecurityModule()` 返回 `address(0)`，则将使用在 [Mailbox](/zh/docs/protocol/core/mailbox) 上配置的默认 ISM 来验证消息。
- 为清晰起见，这在序列图中省略了。
