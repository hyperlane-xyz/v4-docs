...
- **Delivery confirmation** — for non-EVM destinations, the CLI polls the [Hyperlane Explorer](/docs/reference/explorer/overview) for delivery status instead of checking on-chain. Track delivery at `https://explorer.hyperlane.xyz/message/<messageId>`.
...
In multi-hop paths (e.g., `--chains ethereum,polygon,solana`), non-EVM chains can only appear as the **final destination**, not as intermediate hops. When no `--origin`/`--destination` is specified, the CLI automatically orders EVM chains first so non-EVM chains become the final destination.
