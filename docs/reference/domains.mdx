---
title: "Domain Identifiers"
---

## Core Domains[​](#core-domains "Direct link to Core Domains")

This list includes only the deployments on chains that are managed by Abacus Works (our legal entity). Anybody can deploy Hyperlane on any chain [permissionlessly](/docs/guides/chains/deploy-hyperlane) - we are building a registry to reflect the community's permissionless deployments.

Hyperlane uses unique IDs for each Hyperlane-supported chain. When possible Hyperlane domain IDs match EVM chain IDs, but this is **not guaranteed** as we support non-EVM chains as well.

<Warning>
  Some Hyperlane domain IDs do not match their corresponding EVM chain IDs, such as:

- `immutablezkevmmainnet`
- `rarichain`
- `rootstockmainnet`
- `alephzeroevmmainnet`
- `chilizmainnet`
- `lumiaprism`
- `superpositionmainnet`
- `flowmainnet`
- `metal`
- `polynomialfi`

Please refer to the table below for the full list of Hyperlane domain IDs.

</Warning>

When [sending a message](/docs/your-first-message), users must provide the domain ID of the destination chain. When [receiving a message](/docs/reference/messaging/receive), the recipient will be passed the domain ID of the origin chain.

- Mainnet
- Testnet

| Network               | Domain Identifier (uint32) |
| --------------------- | -------------------------- |
| Abstract              | 2741                       |
| Aleph Zero EVM        | 1000041455                 |
| Ancient8              | 888888888                  |
| ApeChain              | 33139                      |
| AppChain              | 466                        |
| Arbitrum              | 42161                      |
| Arbitrum Nova         | 42170                      |
| Arcadia               | 4278608                    |
| Artela                | 11820                      |
| Arthera               | 10242                      |
| Astar                 | 592                        |
| Astar zkEVM           | 3776                       |
| Aurora                | 1313161554                 |
| Avalanche             | 43114                      |
| B3                    | 8333                       |
| Base                  | 8453                       |
| Berachain             | 80094                      |
| Bitlayer              | 200901                     |
| Blast                 | 81457                      |
| BOB                   | 60808                      |
| Boba Mainnet          | 288                        |
| BounceBit             | 6001                       |
| Binance Smart Chain   | 56                         |
| B² Network            | 223                        |
| Celo                  | 42220                      |
| CheeseChain           | 383353                     |
| Chiliz                | 1000088888                 |
| Conflux eSpace        | 1030                       |
| Conwai                | 668668                     |
| Core                  | 1116                       |
| Corn                  | 21000000                   |
| Coti                  | 2632500                    |
| Cyber                 | 7560                       |
| Deep Brain Chain      | 19880818                   |
| Degen                 | 666666666                  |
| Dogechain             | 2000                       |
| DuckChain             | 5545                       |
| Eclipse               | 1408864445                 |
| Endurance             | 648                        |
| Ethereum              | 1                          |
| Everclear             | 25327                      |
| Evmos EVM             | 9001                       |
| Fantom Opera          | 250                        |
| Flame                 | 253368190                  |
| Flare                 | 14                         |
| EVM on Flow           | 1000000747                 |
| Fluence               | 9999999                    |
| Form                  | 478                        |
| Fraxtal               | 252                        |
| Fuse                  | 122                        |
| Game7                 | 2187                       |
| Glue                  | 1300                       |
| Gnosis                | 100                        |
| Gravity Alpha Mainnet | 1625                       |
| Guru Network          | 260                        |
| Harmony One           | 1666600000                 |
| Hashkey               | 177                        |
| Hemi Network          | 43111                      |
| HyperEVM              | 999                        |
| Immutable zkEVM       | 1000013371                 |
| Injective EVM         | 2525                       |
| Infinity VM           | 1032009                    |
| Infinity VM           | 1001032009                 |
| Injective             | 6909546                    |
| Ink                   | 57073                      |
| Kaia                  | 8217                       |
| Kroma                 | 255                        |
| Linea                 | 59144                      |
| Lisk                  | 1135                       |
| LUKSO                 | 42                         |
| Lumia Prism           | 1000073017                 |
| Manta Pacific         | 169                        |
| Mantle                | 5000                       |
| Matchain              | 698                        |
| Merlin                | 4200                       |
| Metal L2              | 1000001750                 |
| Metis Andromeda       | 1088                       |
| MilkyWay              | 1835625579                 |
| Mint                  | 185                        |
| Miraclechain          | 92278                      |
| Mode                  | 34443                      |
| Molten                | 360                        |
| Moonbeam              | 1284                       |
| Morph                 | 2818                       |
| Nero                  | 1689                       |
| Neutron               | 1853125230                 |
| Nibiru                | 6900                       |
| Ontology              | 58                         |
| Oort                  | 970                        |
| opBNB                 | 204                        |
| Optimism              | 10                         |
| Orderly L2            | 291                        |
| Peaq                  | 3338                       |
| Plume                 | 98866                      |
| Polygon               | 137                        |
| Polygon zkEVM         | 1101                       |
| Polynomial            | 1000008008                 |
| Prom                  | 227                        |
| Proof of Play Apex    | 70700                      |
| RARI Chain            | 1000012617                 |
| Reactive Mainnet      | 1597                       |
| re.al                 | 111188                     |
| Redstone              | 690                        |
| Rivalz                | 753                        |
| Ronin                 | 2020                       |
| Rootstock             | 1000000030                 |
| Sanko                 | 1996                       |
| Scroll                | 534352                     |
| Sei                   | 1329                       |
| Shibarium             | 109                        |
| SnaxChain             | 2192                       |
| Solana                | 1399811149                 |
| Soneium               | 1868                       |
| Sonic                 | 146                        |
| Sonic SVM             | 507150715                  |
| SOON                  | 50075007                   |
| Sophon                | 50104                      |
| Story Mainnet         | 1514                       |
| Subtensor             | 964                        |
| Superposition         | 1000055244                 |
| Superseed             | 5330                       |
| svmBNB                | 574456                     |
| Swell                 | 1923                       |
| Taiko                 | 167000                     |
| Tangle                | 5845                       |
| Telos EVM             | 40                         |
| Torus                 | 21000                      |
| Treasure              | 61166                      |
| TRUMPCHAIN            | 4547                       |
| Unichain              | 130                        |
| Unit Zero             | 88811                      |
| Vana                  | 1480                       |
| Viction               | 88                         |
| World Chain           | 480                        |
| Xai                   | 660279                     |
| XLayer                | 196                        |
| XPLA                  | 37                         |
| Zero Network          | 543210                     |
| ZetaChain             | 7000                       |
| Zircuit               | 48900                      |
| zkLink Nova           | 810180                     |
| zkSync                | 324                        |
| Zora                  | 7777777                    |

| Network                   | Domain Identifier (uint32) |
| ------------------------- | -------------------------- |
| Abstract Testnet          | 11124                      |
| Aleph Zero EVM Testnet    | 2039                       |
| Alfajores                 | 44787                      |
| Arbitrum Sepolia          | 421614                     |
| Arcadia Testnet v2        | 1098411886                 |
| Aurora Testnet            | 1313161555                 |
| Basecamp Testnet          | 1000001114                 |
| Base Sepolia              | 84532                      |
| Berachain Bepolia         | 80069                      |
| BSC Testnet               | 97                         |
| CarrChain Testnet         | 76672                      |
| Chronicle Yellowstone     | 175188                     |
| Citrea Testnet            | 5115                       |
| Coti Testnet              | 7082400                    |
| Eco Testnet               | 471923                     |
| Flame Dawn-1 Testnet      | 1660473773                 |
| Form Testnet              | 132902                     |
| Fuji                      | 43113                      |
| Holesky                   | 17000                      |
| Hyperliquid EVM Testnet   | 998                        |
| Infinity VM Monza Testnet | 96025                      |
| Ink Sepolia               | 763373                     |
| MegaETH Testnet           | 6342                       |
| Milkyway Testnet          | 1162171030                 |
| Mode Testnet              | 919                        |
| Monad Testnet             | 10143                      |
| Odyssey Testnet           | 911867                     |
| Optimism Sepolia          | 11155420                   |
| Plume Testnet             | 98867                      |
| Polygon Amoy              | 80002                      |
| Scroll Sepolia            | 534351                     |
| Sepolia                   | 11155111                   |
| Solana Testnet            | 1399811150                 |
| Somnia Testnet            | 50312                      |
| Soneium Minato Testnet    | 1946                       |
| Sonic Blaze Testnet       | 57054                      |
| Sonic SVM Testnet         | 15153042                   |
| Sonic Testnet             | 64165                      |
| SUAVE Toliman Testnet     | 33626250                   |
| Subtensor Testnet         | 945                        |
| Superposition Testnet     | 98985                      |
| Tangle Testnet            | 3799                       |
| Treasure Topaz Testnet    | 978658                     |
| Unichain Testnet          | 1301                       |
| Load Network Alphanet     | 9496                       |
