...
`RateLimitedIsm` can be nested inside an `AggregationIsm` to layer rate limiting on top of other security checks — for example, requiring both a `MultisigIsm` and a `RateLimitedIsm` to pass before a message is delivered. This requires the latest relayer version.
...
[`NetFlowRateLimitedHookIsm`](https://github.com/hyperlane-xyz/hyperlane-monorepo/blob/net-flow-rate-limit-ism/solidity/contracts/hooks/warp-route/NetFlowRateLimitedHookIsm.sol) is an alternative to the `RateLimitedIsm` and `RateLimitedHook` pair that takes a different approach on two fronts:

- **Dynamic cap instead of fixed.** Rather than a fixed `maxCapacity` set by the owner, the cap is derived from the warp route's live TVL: `maxCapacity = localCollateral() × maxFlowBps / 10000`. As TVL grows or shrinks, the effective cap scales with it automatically. `maxFlowBps` is immutable — set once at deployment.

- **Net flow, not gross outflow.** The bucket tracks *net* outflow. Inbound transfers (tokens returning to the route) credit the bucket back, so the limit applies to net outflow over the window rather than total volume sent in one direction. This makes it more permissive for routes with high bidirectional traffic.

- **Single contract.** Hook and ISM logic live in one deployment, wired to the same router address on both sides.

Use `NetFlowRateLimitedHookIsm` when TVL is expected to change significantly over time and you want the cap to scale proportionally. Use `RateLimitedIsm` when you want an explicit fixed cap that the warp route owner controls directly.
