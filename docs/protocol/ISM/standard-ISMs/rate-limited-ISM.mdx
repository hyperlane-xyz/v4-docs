...
- **Dynamic cap instead of fixed.** Rather than a fixed `maxCapacity` set by the owner, the cap is derived from the warp route's live TVL: `maxCapacity = localCollateral() × maxFlowBps / 10_000` (where `10_000` is `BPS_DENOMINATOR`). As TVL grows or shrinks, the effective cap scales with it automatically. `maxFlowBps` is immutable — set once at deployment.
