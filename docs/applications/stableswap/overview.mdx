---
title: "StableSwap Overview"
---

<Warning>
  StableSwap is under active development. Interfaces, fees, and supported routes
  may change.
</Warning>

StableSwap lets users move between like assets (most commonly stablecoins) across different chains in a single step.

For example: a user holds USDC on Arbitrum and wants a branded stablecoin on a new chain. With StableSwap, that happens in one transaction. No manual bridging, no extra steps.
...
Without something like StableSwap, a user who wants to move from USDC into a new branded stablecoin has to:
...
StableSwap replaces all of this with a single transaction. The user sends USDC from any supported chain and receives the branded stablecoin on the destination chain. There is no bridge step, no DEX, and no need to bootstrap liquidity. The tokens held inside the routers _are_ the liquidity.
...
- **Cross-chain and same-chain swaps** in a single transfer model
- **Per-route fees:** protocol and external fees are configured per destination domain and per target router, so each leg of a route can carry its own pricing
- **Native rebalancing**: inventory is automatically balanced across chains to keep routes available
...
2. StableSwap routes it to the destination chain.
...
StableSwap is built on top of [Hyperlane Warp Routes 2.0](/docs/applications/warp-routes/multi-collateral-warp-routes).

Each chain in a StableSwap route has a **`CrossCollateralRouter`** deployed for each collateral token (e.g. one for USDC, one for the branded stable). `CrossCollateralRouter` extends `HypERC20Collateral`, so each instance holds collateral for exactly one ERC20.
...
1. User calls `transferRemoteTo` on the source chain router with the destination domain, recipient, amount, and target router.
2. The source router validates that the target router is enrolled, deducts configured fees, and dispatches a single interchain message via Hyperlane.
3. The destination router receives the message, verifies the sender is a trusted router, and releases the target stablecoin to the recipient.
...
To learn more about the underlying architecture or explore what a route deployment looks like, see:

- [Hyperlane Warp Routes 2.0](/docs/applications/warp-routes/multi-collateral-warp-routes)
- [Deploy a Hyperlane Warp Route 2.0](/docs/guides/warp-routes/evm/deploy-multi-collateral-warp-routes)
- [Native Rebalancing](/docs/guides/warp-routes/evm/multi-collateral-warp-routes-rebalancing)
