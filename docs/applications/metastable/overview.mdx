...
Metastable is an on-chain clearing solution for stablecoins. It instantly converts one stablecoin into another across chains, in a single transaction. Send USDC on one chain, receive a different stablecoin on another, with no bridging, no DEX, and no slippage.

Any stablecoin (USDC, USDT, or a branded stablecoin) can be enrolled.
