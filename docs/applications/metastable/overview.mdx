...
Metastable instantly converts one stablecoin into another across chains — in a single transaction. Send USDC on one chain, receive a different stablecoin on another, with no bridging, no DEX, and no slippage. Any stablecoin — USDC, USDT, or a branded stablecoin — can be enrolled.
...
Take a branded stablecoin. Launching one is now easy; getting holders to actually use it is the hard part. Most holders already keep their balances in USDC or USDT, spread across many chains — so to grow, a new stablecoin has to meet them where their funds already are.
...
- **Flexible pricing**: fees are set per route, so the team can charge on the way in, on the way out, on both, or set fees to zero to encourage adoption. Fees can also be quoted off-chain, so the team can give chosen users a preferential rate such as 1:1, while everyone else pays the standard route fee

## Use Cases

<CardGroup cols={1}>
  <Card title="Branded stablecoin issuers" icon="building-columns">
    Metastable handles conversions between the branded stablecoin and broad-market
    stablecoins like USDC and USDT.
  </Card>
  <Card title="Stablecoin payment apps" icon="credit-card">
    A payment app may accept deposits in one stablecoin, hold balances in another,
...
<CardGroup cols={2}>
  ...
  <Card title="Per-route fees" icon="coins">
    ...
    all. Fees can also be quoted off-chain to give chosen users a lower or fee-free
    rate.
  </Card>
  <Card title="Automatic rebalancing" icon="scale-balanced">
    ...
    chains automatically — no active management needed.
  </Card>
