...
Metastable is a way to swap between like assets (most commonly stablecoins) across different chains in a single step. Any stablecoin — USDC, USDT, or a branded stablecoin — can be enrolled.
...
- **Flexible pricing**: fees are set per route, so the issuing team can charge on the way in, on the way out, on both, or set fees to zero to encourage adoption. Fees can also be quoted off-chain, so the team can give chosen users — for example retail — a preferential rate such as 1:1, while everyone else pays the standard route fee
...
<AccordionGroup>
  ...
  <Accordion title="Are fees configurable?">
    ...
    signed quotes to give specific users (for example retail) a preferential
    rate such as 1:1, while everyone else pays the standard route fee.
  </Accordion>
