...
Native rebalancing is only possible when the required conditions are met for a given token on each participating chain in the HWR 2.0.
...
<Note>
  ...
  rebalancing.
  ](/docs/guides/warp-routes/evm/multi-collateral-warp-routes-rebalancing#requirements-for-native-rebalancing)
</Note>

### USDC

- **Protocol**: [Circle CCTP](https://developers.circle.com/cctp)
