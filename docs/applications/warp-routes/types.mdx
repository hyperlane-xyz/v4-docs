...
Implemented in `HypERC20.sol`, synthetic HWR mint new tokens on the destination chain that represent the original tokens from the source chain. The original tokens are not transferred but remain locked.
...
All HWR extend the `TokenRouter` contract, which provides the core functionality for warp route token transfers.
...
For setup examples & use cases, check out [Hyperlane Warp Routes: Example Usage](./example-usage.mdx).
