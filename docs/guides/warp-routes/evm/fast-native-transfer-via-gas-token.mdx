...
4. After execution, your deployment config in `$HOME/.hyperlane/deployments/warp_routes/ETH/ethereum-yourchain/` will look similar to the following:
...
```bash
hyperlane warp send --relay -w ETH/ethereum-yourchain
```
...
```bash
hyperlane relayer -w ETH/ethereum-yourchain
```

You can test in either direction between where you have the HWR set.

<Info>
  By default, your warp route config is found in the local registry at
  `$HOME/.hyperlane/deployments/warp_routes/<warpRouteId>`.
</Info>
