...
4. After execution, your deployment config in `$HOME/.hyperlane/deployments/warp_routes/ETH/yourid/` will look similar to the following:
...
```bash
hyperlane warp send --relay -w ETH/yourid
```
...
```bash
hyperlane relayer -w ETH/yourid
```
