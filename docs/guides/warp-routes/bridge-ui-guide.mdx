...
After you've successfully [deployed a Hyperlane Warp Route (HWR)](/docs/guides/quickstart/deploy-warp-route.mdx), you have two out-of-the-box options for setting up a frontend UI to support interchain token transfers:
...
Chain metadata can also be configured as needed. These are the same configs as those used with the CLI for any of its deploy or send command (for example, those from the [deploy guide](/docs/guides/chains/deploy-hyperlane.mdx)).
...
A common problem with token bridges like Hyperlane HWRs is that a user may transfer a token like USDC to a new chain, but only afterwards realize that they do not have the native gas token to move those tokens anywhere including back.
...
They've also built a handy feature that allows for self-service testing of recently deployed Hyperlane HWRs.
...
3. Paste in the YAML file you generated when deploying your Hyperlane HWRs ![](/images/docs/guides/deploy-warp-ui/superbridge-paste-yaml.png)
