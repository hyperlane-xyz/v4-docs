...
ethereum:
  ...
  gas: 300000 # Compute units for message delivery on Cosmos
...
- **`gas`**: In the EVM config, the `gas` field specifies the compute units required for message delivery on the destination Cosmos chain.
...
<Note>
**Private Key Format:** Both EVM and Cosmos keys should be provided in **hex format** (64 characters, no `0x` prefix). The CLI handles the derivation of the bech32 address for Cosmos automatically.
</Note>
...
```bash
hyperlane warp read --warp USDC/ethereum
```
...
To ensure everything is working correctly, send a test transfer between the chains:

```bash
hyperlane warp send --origin ethereum --destination yourcosmoschain --relay
```

The `--relay` flag ensures the message is delivered to the destination chain immediately by the CLI.
...
### Gas and Fees
When sending a message from EVM to Cosmos, you must specify a sufficient `gas` limit in your config. This represents the compute units on the Cosmos side. Ensure your EVM wallet has enough native tokens to pay for the interchain gas fees.
