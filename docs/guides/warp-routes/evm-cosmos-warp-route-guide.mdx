...
<Note>
  `hyperlane warp send` does not currently support Cosmos SDK chains. To test your cross-chain warp route, run a [relayer](/docs/operate/relayer/run-relayer) between your chains and initiate a transfer from the EVM side using the deployed HypERC20 contract, or from the Cosmos side using native transaction tooling.
</Note>
