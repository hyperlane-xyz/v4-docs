...
- [Hyperlane CLI](/docs/reference/developer-tools/cli.mdx)
...
7. Deploy the Ethereum HWR contract, which will also enroll the SVM router based on the yaml token config.
   ...
   1. Ensure you have latest version of the [Hyperlane CLI](/docs/reference/developer-tools/cli.mdx).
   2. Create a HWR deployment config file with the CLI's config command: `hyperlane warp init `
      ...
      - Set the `gas` to a ceiling of the compute units you expect the SVM message delivery transaction to take. For instance, Hyperlane HWRs have gas set to `300`. It's important for this to be an upper limit - the relayer will not deliver HWR transfer messages otherwise, because senders would pay an insufficient amount to have them delivered.
