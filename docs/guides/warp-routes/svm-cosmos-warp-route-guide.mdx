---
title: "Deploy an SVM ↔ Cosmos HWR"
---

This guide walks you through deploying a Hyperlane Warp Route (HWR) between an SVM chain (Solana, Eclipse) and a Cosmos SDK chain. It combines the Sealevel Rust CLI for the SVM side with the Hyperlane TypeScript CLI for the Cosmos side, linked via the `foreignDeployment` field.
...
The CLI will output deployment artifacts to `~/.hyperlane/deployments/warp_routes/`. Note the deployed Warp Route address — you'll need it for the SVM side.
...
```json
{
  "solanamainnet": {
    ...
    "interchainGasPaymaster": "<from SVM core artifacts, use the overhead IGP>"
  },
  "yourcosmoschain": {
    "type": "collateral",
    "decimals": 6,
...
<Note>
  The recipient address must be in hex format (not bech32). The relayer handles the conversion to the Cosmos bech32 address format on the destination chain.
</Note>
...
When sending messages from SVM to Cosmos, the IGP on the SVM side handles gas payments. Ensure the IGP is properly configured with oracle prices for the Cosmos destination chain.

When sending from Cosmos to SVM, ensure the Cosmos-side IGP has oracle prices configured for the SVM destination domain.
