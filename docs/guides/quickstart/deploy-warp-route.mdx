...
- _Optional:_ Hyperlane connection details, such as contract addresses for the [Mailbox](/docs/reference/addresses/deployments/mailbox), [Interchain Gas](/docs/reference/hooks/interchain-gas), and [Interchain Security Modules](/docs/protocol/ISM/modular-security#core-concepts).
...
The warp route ID follows the format `SYMBOL/chain1-chain2` (e.g., `ETH/ethereum-yourchain`). This ID is used with CLI commands like `warp read`, `warp send`, and `warp apply`.

<Info>
All warp route configs are now stored in the registry. To modify a warp route config, edit the files in `$HOME/.hyperlane/deployments/warp_routes/<warpRouteId>/` and then run `hyperlane warp apply`.
</Info>
...
```
hyperlane warp send --relay --warp-route-id TOKEN/chain1-chain2
```
...
```
hyperlane relayer --warp-route-id TOKEN/chain1-chain2
```
